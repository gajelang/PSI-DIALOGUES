...
const Recommendation = () => {
    ...
    useEffect(() => {
        const fetchRecommendations = async () => {
            try {
                const response = await fetch('https://dialogue-api.vercel.app/api/userRec/userec?userID=1');
                if (!response.ok) {
                    throw new Error('Failed to fetch recommendations');
                }
                const data = await response.text();
                console.log('Recommendations:', data); // Logging the fetched data
                setRecommendations(data); // Assuming recommendations is an array in the API response
    ...
    return (
        <div className="flex justify-center mt-10 gap-8 rounded-lg">
            <img className='w-1/4' src='../../img/AIBOT.png'></img>
            <div>
                <div className='w-full bg bg-[#474af1] rounded-md flex flex-row items-center px-6 py-3 gap-4'>
                    ...
                    <h1 className='text-2xl font-bold text-white'>Performance Review by AI</h1>
                </div>
